        ...
        while (*src && count < len) {
            if (*src != ' ' && *src != '\t') {
                *dest++ = *src++;
                count++;
                space_added = 0;
            } else if (!space_added && count < len - 1) {
                *dest++ = ' ';
                count++;
                space_added = 1;
                src++;
    ...
    void reverse_string(char *buff, int str_len) {
        if (!buff || str_len <= 0) {
            printf("Error: Invalid buffer or string length.\n");
            return;
        }
    ...
    int replace_word(char *buff, int str_len, char *target, char *replacement) {
        ...
        while (pos < buff + str_len - target_len + 1) {
            if (*pos == *target && strncmp(pos, target, target_len) == 0) {
                ...
                for (int i = new_str_len; i < BUFFER_SZ; i++) {
                    buff[i] = '.';
                }
    ...
    int main(int argc, char *argv[]){
        ...
        switch (opt){
            ...
            case 'x': 
            {
                ...
                if (rc == -2) { // Handle the "Not Implemented" case
                    free(buff);
                    exit(1); // Exit without printing the buffer
                }
                user_str_len = rc;
                break;
            }
